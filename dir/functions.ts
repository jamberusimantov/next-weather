...
const fetchData = async (city: string, unit: string) => {
    ...
    try {
        ...
        result.list.forEach((timeForecastObj: hourly) => {
            ...
            dailyArr[counter].icons.forEach((iconObj: iconObj) => {
                if (!isMatch && iconObj.icon === timeForecastObj.weather[0].icon) {
                    iconObj.counter += 1;
                    isMatch = true;
                }
            })
            if (!isMatch) dailyArr[counter].icons.push({
                icon: timeForecastObj.weather[0].icon,
                counter: 1,
            })
        })
...
const getSpeed = (speed: number, unit: string) => {
    return unit === "imperial" ? `${~~(speed * 2.24)}mph` : `${~~speed}ms`;
}
...
export { fetchData, getTemp, getSpeed, getDay, getDaily }